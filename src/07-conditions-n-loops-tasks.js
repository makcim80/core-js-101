...
function getSumBetweenNumbers(n1, n2) {
  return Array.from({ length: n2 - n1 + 1 }, (_, k) => k + n1).reduce((a, c) => a + c);
}
...
function isCreditCardNumber(ccn) {
  return !(ccn.toString().split('').reverse().reduce((acc, cur, i) => {
    if (i % 2) {
      return +acc + +(cur > 4 ? cur * 2 - 9 : cur * 2);
    }
    return +acc + +cur;
  }) % 10);
}
...
function getDigitalRoot(num) {
  const result = num.toString().split('').reduce((acc, cur) => +acc + +cur);
...
function getCommonDirectoryPath(pathes) {
  const arrNew = pathes.map((elem) => elem.split(/(?<=\/)/));
  return arrNew[0].filter((e, index) => arrNew.every((a) => a[index] === e)).join('');
}
...
function getMatrixProduct(m1, m2) {
  const matrix = Array
    .from({ length: m1.length }, () => Array.from({ length: m2[0].length }, () => 0));
  for (let i = 0; i < matrix.length; i += 1) {
    for (let j = 0; j < matrix[0].length; j += 1) {
      for (let n = 0; n < m1[0].length; n += 1) {
        matrix[i][j] += m1[i][n] * m2[n][j];
      }
    }
  }
  return matrix;
...
function evaluateTicTacToePosition(p) {
  if (p[0][0]) {
    if (p[0][0] === p[0][1] && p[0][1] === p[0][2]) return p[0][0];
    if (p[0][0] === p[1][0] && p[1][0] === p[2][0]) return p[0][0];
  }
  if (p[1][1]) {
    if (p[0][0] === p[1][1] && p[1][1] === p[2][2]) return p[1][1];
    if (p[2][0] === p[1][1] && p[1][1] === p[0][2]) return p[1][1];
    if (p[1][0] === p[1][1] && p[1][1] === p[1][2]) return p[1][1];
    if (p[0][1] === p[1][1] && p[1][1] === p[2][1]) return p[1][1];
  }
  if (p[2][2]) {
    if (p[2][0] === p[2][1] && p[2][1] === p[2][2]) return p[2][2];
    if (p[0][2] === p[1][2] && p[1][2] === p[2][2]) return p[2][2];
  }
  return undefined;
}
