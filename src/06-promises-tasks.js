...
function willYouMarryMe(isPositiveAnswer) {
  const strPositive = 'Hooray!!! She said "Yes"!';
  const strNegative = 'Oh no, she said "No".';
  const strError = 'Wrong parameter is passed! Ask her again.';
  return new Promise((resolve, reject) => {
    if (typeof isPositiveAnswer === 'boolean') {
      resolve(isPositiveAnswer ? strPositive : strNegative);
    }
...
async function chainPromises(array, action) {
  return array
    .reduce(async (accumulator, currentValue) => {
      try {
        return action(await accumulator, await currentValue);
      } catch (e) {
        return e;
      }
    });
